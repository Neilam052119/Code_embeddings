...
# final_excel_path = os.path.join(output_folder, 'keywords_with_visuals.xlsx')
...
# # Function to check for keywords using fuzzy matching
...
# # Function to save extracted words
...
#         # Save OCR result visualization for each image
...
#         visual_image_path = os.path.join(output_folder, f'result_{image_file}')
#         image_with_boxes.save(visual_image_path)
        
#         # Append results
#         all_data.append({"Image": image_file, "Contains Keywords": contains_keywords, "Visualized Image": visual_image_path})

# # Save final results to Excel including the path to the visualized image
# df = pd.DataFrame(all_data)
# df.to_excel(final_excel_path, index=False)
# print(f"Final OCR results and visualized image paths have been saved to {final_excel_path}")

# # Save extracted words to .txt and .json files
# save_extracted_words(extracted_words, os.path.join(output_folder, 'extracted_words.txt'), 'txt')
# save_extracted_words(extracted_words, os.path.join(output_folder, 'extracted_words.json'), 'json')
...
# Define paths
base_input_folder = r'C:\Users\neela\OneDrive\Desktop\Code_embeddings\Delhi\JULY'
output_folder = r'C:\Users\neela\OneDrive\Desktop\Code_embeddings\output'
...
# Loop through all "Extracted_Images" folders
for folder_name in os.listdir(base_input_folder):
    folder_path = os.path.join(base_input_folder, folder_name)
    
    if os.path.isdir(folder_path) and folder_name.startswith("Extracted_image_"):
        # Find the "crop image" folder
        crop_image_folder = os.path.join(folder_path, "crop image")
        
        if os.path.exists(crop_image_folder):
            # Find the "articles" folder inside "crop image"
            articles_folder = os.path.join(crop_image_folder, "articles")
            
            if os.path.exists(articles_folder):
                print(f"Processing folder: {articles_folder}")
                
                for image_file in os.listdir(articles_folder):
                    image_path = os.path.join(articles_folder, image_file)
